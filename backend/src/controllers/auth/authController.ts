...
export const signupController = publicProcedure
  .input(authSchema)
...
export const verifyOTPController = publicProcedure
  .input(emailOTPSchema)
  .mutation(async ({ input }) => {
    return asyncHandler(async () => {
      const findExistUser = (await User.findOne({
        where: {
      ...
      if (findExistUser.otp_expiry) {
        ...
        const isOTPExpired =
          new Date().getSeconds() > findExistUser.otp_expiry.getSeconds();
